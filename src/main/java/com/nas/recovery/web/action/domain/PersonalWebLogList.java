...
@Name("personalWebLogList")
@Scope(ScopeType.CONVERSATION)
public class PersonalWebLogList extends WebLogListQueryBase implements java.io.Serializable {

    @In(create = true)
    protected EntityManager entityManager;
    EmployeeAction employeeAction=(EmployeeAction) Component.getInstance("employeeAction");
    
    List<WebLog> personalWeblogs;
    ...
    synchronized public void fetchnewPersonalWeblogs() {
        ...
        List<WebLog> result = q.setParameter("logTag", webLogType.Private).setParameter("tag", "nrs").setParameter("logEmp", employeeAction.getCurrentLoggedInEmployee()).getResultList();
        personalWeblogs = result;
